...
function isVideoUrl(url: string) {
  return /youtube|youtu\.be|vimeo|\.mp4|\.webm|\.mov/i.test(url);
}
...
export default function SightingsTab() {
  ...
  const fetchSightings = useCallback(async (sort: string) => {
    setLoading(true);
    try {
      const res = await fetch(`/api/sightings?sort=${sort}`);
      const data = await res.json();
      if (Array.isArray(data)) setSightings(data);
    } finally {
      setLoading(false);
    }
  }, []);

  useEffect(() => { fetchSightings(subTab); }, [subTab, fetchSightings]);
